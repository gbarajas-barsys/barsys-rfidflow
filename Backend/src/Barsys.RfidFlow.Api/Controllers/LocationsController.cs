...
public sealed class LocationsController : ApiControllerBase
{
    ...
    public LocationsController(IRepository<Location> repository) => _repository = repository;
    ...
    [HttpPost]
    public async Task<IActionResult> Create(Location entity, CancellationToken ct)
    {
    ...
    [HttpPatch("{id:guid}")] 
    public async Task<IActionResult> Patch(Guid id, Location patch, CancellationToken ct)
    {
        var updated = await _repository.UpdateAsync(TenantId, id, current =>
        {
            ...
            foreach (var p in typeof(Location).GetProperties().Where(p => p.CanWrite && p.Name != "Id" && p.Name != "TenantId"))
            {
